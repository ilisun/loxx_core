...
- [x] Написать парсер OSM (PBF) и конвертер в тайлы (land graph) → FlatBuffers.
- [x] Реализовать C++ ядро: структуры графа, чтение тайлов, bi-A*.
- [x] Реализовать CarProfile и FootProfile (скорости, доступность).
- [x] C++ API `route(start,end,profile)` → polyline+длина+ETA.
- [x] Собрать оффлайн-пакет для одного региона (Северо-Западный ФО, 294K тайлов, 1.2 ГБ).
- [x] Собрать iOS SDK (Swift Package) с чистым Swift API, MapLibre интеграцией и документацией.
